...
<body>
	...
	<div id="trainOptions">
		<form action="./search" method="GET">
			...
			</select> <input type="submit" value="Filter Train">
		</form>
	</div>
	<%
            ...
            for(Train train:trainList){
				...
				if(!trainsdao.ifSeatDayExist(travelDate,train.getName())) {
					
					trainsdao.createSeat(travelDate, train.getACCompartmentNo()*train.getACCompartmentSeats(), train.getNONACCompartmentNo()*train.getNONACCompartmentSeats(), train.getName());
				}
            }
	...
	<table>
        ...
        %>
			<tr>
				...
				<td><%= trainsdao.getSeats("ACseats",train.getName(),(String)session.getAttribute("travelDate")) %></td>
				<td><%= trainsdao.getSeats("NONACseats",train.getName(),(String)session.getAttribute("travelDate")) %></td>
			</tr>
			<%
                }
		%>
	...
	<form action="./book" method="POST">
		<label for="train">Trains:</label> <select id="train" name="train"
			required>
			<option value="">Select Time</option>
			<%
        ...
        %>
			...
			%>
		</select> <label for="numOfTravelers">No Of Travelers:</label> <input
			type="number" id="numOfTravelers" name="numOfTravelers" min=1 max=5
			required> <label for="compartment">Compartment:</label> <select
			...
			<option value="NONACseats">Non AC</option>
		</select> <input type="submit" value="Select Train">
	</form>
	<%	
