...
public class PDFDecorator {
	...
	public void createTicket(TicketBatch batch) {
		...
		String outputPath = "/home/kabilan-22527/eclipse-workspace/railway/src/main/resources/ticket.pdf";
		...
		try {
			...
			Paragraph title = new Paragraph("Railway Ticket").setFontSize(15).setBold()
			...
			document.add(
					createTicketInfo("SOURCE: " + stationsdao.getStation(batch.getSource()).getName().toUpperCase()));
			document.add(createTicketInfo(
					"DESTINATION: " + stationsdao.getStation(batch.getDestination()).getName().toUpperCase()));
			...
			Paragraph footer = new Paragraph("Thank you for Travelling!").setFontSize(12)
					.setTextAlignment(TextAlignment.CENTER);
			document.add(footer);
